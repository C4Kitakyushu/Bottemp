...
module.exports = {
  name: "dehaze",
  usage: "dehaze (must be used with an image attachment or reply to an image)",
  description: "Enhance an image using AI via the Dehaze API.",
  ...
  execute: async ({ api, event, args }) => {
    ...
    if (messageReply && messageReply.attachments.length > 0) {
      ...
      return api.sendMessage(
        "❌ Please send an image or reply to an image with the message `dehaze` to enhance it.",
        threadID,
        messageID
      );
    }
    ...
    try {
      const enhanceUrl = `https://xnilnew404.onrender.com/xnil/remini?imageUrl=${encodeURIComponent(imageUrl)}&method=dehaze`;
